...
В качестве инструмента организации микрофронтендов выбран **ModuleFederation** так как позволяет использовать в коде общие функции (PopupWithForm), а использование разных 
фреймворков для разработки микрофронтендов (что послужило бы поводом для выбора SingleSPA) задачей не предусмотрено и смысла переделывать их не прорисовывается.
...
Также в репозитории - Розаренов А.В. arch_template_task2.drawio
